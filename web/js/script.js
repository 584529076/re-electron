...
const state = {
 ...
 // ========= 新增：AI 工具列表缓存（动态 lightbox 按钮用）=========
 _aiTools: [],
 _aiToolsLoaded: false,
 // ========= 新增：Module 2 元信息收起状态（默认收起）=========
 _metaCollapsed: true,
 // ========= 新增：Module 3 标签收起状态（默认收起）=========
 _tagsCollapsed: true,
};
...
// ========== 预览图压缩：超过 maxBytes 时降质 + 缩边，循环到目标大小 ==========
window._compressImageToBase64 = async function (file, maxBytes = 100 * 1024) {
    const _fileToBase64 = (f) => new Promise((res, rej) => {
        const fr = new FileReader();
        fr.onload = () => res(((fr.result || '').split(',')[1] || ''));
        fr.onerror = () => rej(fr.error);
        fr.readAsDataURL(f);
    });
    const _blobToBase64 = (b) => new Promise((res, rej) => {
        const fr = new FileReader();
        fr.onload = () => res(((fr.result || '').split(',')[1] || ''));
        fr.onerror = () => rej(fr.error);
        fr.readAsDataURL(b);
    ...
    if (file.size <= maxBytes) {
        return { dataBase64: await _fileToBase64(file), mime: file.type, compressed: false };
    }
    // 用 canvas 重画为 jpeg，循环降低 quality / 缩边 直到达标
    ...
    try {
        ...
        while (tried < 15) {
            ...
            ctx.fillStyle = '#ffffff';  // png/webp 透明区域转 jpeg 时填充白底，避免变黑
        ...
        if (!blob) throw new Error('压缩后仍超目标大小');
        return {
            dataBase64: await _blobToBase64(blob),
            mime: 'image/jpeg',
            compressed: true,
...
const els = {
    ...
    lightboxAiToolBtns: document.getElementById('lightboxAiToolBtns'),
    promptAiToolBtns: document.getElementById('promptAiToolBtns'),
    ...
    // Module 2 收起展开
    paneModuleMeta: document.querySelector('.pane-module-meta'),
    metaModuleHeader: document.getElementById('metaModuleHeader'),
    metaToggle: document.getElementById('btnMetaToggle'),
    // Module 3 收起展开
    paneModuleTags: document.querySelector('.pane-module-tags'),
    tagsModuleHeader: document.getElementById('tagsModuleHeader'),
    tagsToggle: document.getElementById('btnTagsToggle'),
...
function init() {
    ...
    // 可折叠模块：Module 2 元信息 + Module 3 标签（默认都收起）
    _initCollapsible(els.paneModuleMeta, els.metaModuleHeader, '_metaCollapsed');
    _initCollapsible(els.paneModuleTags, els.tagsModuleHeader, '_tagsCollapsed');
}

/**
 * 通用：给可折叠模块绑定收起/展开交互
 *  - 默认 state[key] = true（HTML 也预置 .collapsed class，双重保险）
 *  - 点击 header 或 chevron 按钮 → 翻转
 *  - 键盘 Enter / Space 也触发（header 已是 role=button + tabindex=0）
 *  - chevron button tabindex=-1，靠事件冒泡到 header 统一触发，避免重复切换
 */
function _initCollapsible(module, header, stateKey) {
    ...
    const toggle = () => apply(!state[stateKey]);
...
function _setPromptStatus(text, cls) {
    ...
    // 空文本时自动隐藏（避免无意义的 "就绪" 占位；动态反馈时自动出现）
    els.promptStatus.hidden = !text;
}
...
function _renderPromptDisplay() {
    ...
    // Module 4 下方 AI 工具按钮：仅在 _currentPrompt 非空时渲染；empty 时清空
    _renderPromptAiToolButtons();
}
...
/**
 * 懒加载 AI 工具列表（缓存到 state._aiTools）
 */
async function _loadAiTools() {
    if (state._aiToolsLoaded) return state._aiTools;
    if (!_hasElectronApi() || !window.api.tools || typeof window.api.tools.list !== 'function') {
        state._aiTools = [];
        state._aiToolsLoaded = true;
        return state._aiTools;
    }
    try {
        const r = await window.api.tools.list();
        // IPC 返回 { ok, tools }，tools 是数组
        const list = (r && r.ok && Array.isArray(r.tools)) ? r.tools : (Array.isArray(r) ? r : []);
        // 排除 broken / 隐藏字段
        state._aiTools = list.filter(t => !t.broken);
    } catch (e) {
        console.warn('[ai-tools] list 失败:', e);
        state._aiTools = [];
    }
    state._aiToolsLoaded = true;
    return state._aiTools;
...
/**
 * 在 lightbox 顶部根据当前资产类型动态渲染 AI 工具按钮
 * 筛选条件：tool.accepts 包含 assetType
 */
async function _renderAiToolButtons(assetType) {
    const wrap = els.lightboxAiToolBtns;
    if (!wrap) return;
    wrap.innerHTML = '';
    // 没类型（占位符 / 未知资源）直接清空
    if (!assetType || (assetType !== 'image' && assetType !== 'video')) return;
    const tools = await _loadAiTools();
    const matched = tools.filter(t => Array.isArray(t.accepts) && t.accepts.includes(assetType));
    ...
    for (const t of matched) {
        ...
        btn.type = 'button';
        btn.dataset.aitoolId = t.id;
        btn.dataset.aitoolAsset = assetType;
        btn.dataset.aitoolLabel = t.name || t.id;
        btn.title = t.description || (t.name || t.id);
        // 不同输出类型配不同图标
        const outs = t.outputNodeTypes || [];
        const isVid = outs.some(n => n.type === 'video');
        const isText = outs.every(n => n.type === 'text');
        const icon = isText ? 'fa-wand-magic-sparkles' : (isVid ? 'fa-film' : 'fa-palette');
        const label = isText ? '反推提示词' : (isVid ? '生成视频' : '生成图片');
        btn.innerHTML = '<i class="fa-solid ' + icon + '" aria-hidden="true"></i> ' + String(t.name || t.id).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
        // label 短一些，避免长名字挤出按钮
        btn.title = (t.description || label) + '\n（点击：' + label + '）';
...
/**
 * Module 4 提示词下方：根据当前提示词动态渲染 AI 工具按钮
 * 筛选条件：tool.accepts 包含 'text'；仅在 _currentPrompt 非空时渲染
 */
async function _renderPromptAiToolButtons() {
    const wrap = els.promptAiToolBtns;
    if (!wrap) return;
    wrap.innerHTML = '';
    // 仅在有提示词时显示（避免空状态下空按钮占位）
    if (!_currentPrompt || !_currentPrompt.trim()) return;
    const tools = await _loadAiTools();
    const matched = tools.filter(t => Array.isArray(t.accepts) && t.accepts.includes('text'));
    if (matched.length === 0) return;
    const frag = document.createDocumentFragment();
    for (const t of matched) {
        const btn = document.createElement('button');
        ...
        btn.type = 'button';
        btn.dataset.aitoolId = t.id;
        btn.dataset.aitoolAsset = 'text';
        btn.dataset.aitoolLabel = t.name || t.id;
        // text 类工具：输出若是 video 配 fa-film，其他（image/混合）配 fa-palette
        const outs = t.outputNodeTypes || [];
        const isVid = outs.some(n => n.type === 'video');
        const icon = isVid ? 'fa-film' : 'fa-palette';
        const actionLabel = isVid ? '生成视频' : '生成图片';
        const name = String(t.name || t.id).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
        btn.innerHTML = '<i class="fa-solid ' + icon + '" aria-hidden="true"></i> ' + actionLabel + '：' + name;
        btn.title = (t.description || actionLabel) + '\n（点击：' + actionLabel + '，自动把当前提示词作为输入）';
        frag.appendChild(btn);
    }
    wrap.appendChild(frag);
}
...
function bindEvents() {
 ...
 fileName: file.name
    ...
    // Module 1 + Module 4: AI 工具入口（动态渲染，按 schema.accepts 筛选）— 事件代理
    // 统一绑到 previewModal，子容器（lightboxAiToolBtns / promptAiToolBtns）通吃
    if (els.previewModal) {
        els.previewModal.addEventListener('click', (e) => {
            const btn = e.target.closest('button[data-aitool-id]');
            if (!btn) return;
            const toolId = btn.dataset.aitoolId;
            const assetType = btn.dataset.aitoolAsset;
            _jumpToAiToolWithAsset(toolId, assetType, btn.dataset.aitoolLabel || 'AI 工具');
        });
    }
    ...
    // 从任意 tab 弹框跳转到 AI 工具并预填资产（按 schema 字段名自适应）
    // toolId: 目标 schema id（如 'Wan2.2-SmoothMix-I2V' / 'Prompt_Inversion'）
    // assetType: 'image' | 'video'（决定预填什么）
    // actionName: 中文按钮名（用于 toast 显示）
    async function _jumpToAiToolWithAsset(toolId, assetType, actionName) {
        if (!_previewMeta) {
        ...
        // 拉工具完整 schema，找第一个匹配 assetType 的字段作为预填目标
        let tool;
        try {
            const r = await window.api.tools.get(toolId);
            if (!r || !r.ok || !r.tool) {
                showToast(`${actionName}：工具「${toolId}」不可用${r && r.error ? '：' + r.error : ''}`, 'error');
                return;
            }
            tool = r.tool;
        } catch (e) {
            showToast(`${actionName}：${e && e.message || '未知'}`, 'error');
            return;
        }
        ...
        if (assetType === 'image') {
            ...
            if (!imgField) {
                showToast(`${actionName}：工具「${toolId}」没有 image 字段`, 'error');
                return;
            }
            // 把预览图（任意来源）转 dataUrl —— 兼容资产 tab、本地目录、网络 tab、blob URL
            let dataUrl;
            try {
            ...
            showToast(`${actionName}：跳转到 AI 工具「${tool.name || toolId}」...`, 'info');
            try {
                await window.aiTools.jumpToTool(toolId, {
                    [imgField.id]: { type: 'image', dataUrl },
                });
            } catch (e) {
                showToast(`跳转失败: ${e && e.message || '未知'}`, 'error');
            }
        } else if (assetType === 'video') {
            // 视频→视频/视频→图片工作流待实现；此处保留框架
            showToast(`${actionName}：视频→视频工作流待实现`, 'error');
        } else if (assetType === 'text') {
            // 提示词→文本/图片/视频：把当前 _currentPrompt 预填到第一个 textarea/text 字段
            // 若工具同时 accepts:image 且 lightbox 当前是图片，把图片也一起带过去
            const txtField = fields.find(f => f.type === 'textarea' || f.type === 'text');
            if (!txtField) {
                showToast(`${actionName}：工具「${toolId}」没有文本字段`, 'error');
                return;
            }
            const promptText = (_currentPrompt || '').trim();
            if (!promptText) {
                showToast(`${actionName}：当前提示词为空`, 'error');
                return;
            }
            const prefill = { [txtField.id]: { type: 'text', value: promptText } };
            // 双重输入：工具同时接受图片 + 当前 lightbox 是图片 → 顺带预填图片
            const alsoTakesImage = Array.isArray(tool.accepts) && tool.accepts.includes('image')
                && _previewMeta && _previewMeta.type === 'image';
            if (alsoTakesImage) {
                const imgField = fields.find(f => f.type === 'image');
                if (imgField) {
                    try {
                        const dataUrl = await _resolvePreviewImageAsDataUrl();
                        prefill[imgField.id] = { type: 'image', dataUrl };
                    } catch (e) {
                        // 图片预填失败不阻断文本预填，仅警告
                        showToast(`图片预填失败（不影响文本）：${e && e.message || '未知'}`, 'error');
                    }
                }
            }
            _closePreview();
            ...
            showToast(`${actionName}：跳转到 AI 工具「${tool.name || toolId}」...`, 'info');
            try {
                await window.aiTools.jumpToTool(toolId, prefill);
            } catch (e) {
                showToast(`跳转失败: ${e && e.message || '未知'}`, 'error');
            }
        }
